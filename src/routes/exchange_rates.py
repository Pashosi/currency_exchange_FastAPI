...
async def get_exchange_rate(
        codes: Annotated[str, Path(max_length=6, min_length=6, regex="^[A-Z]{6}$")],
...
async def create_exchange_rate(
        base_currency_code: Annotated[str, Query(max_length=3, min_length=3, regex="^[A-Z]{3}$")],
        target_currency_code: Annotated[str, Query(max_length=3, min_length=3, regex="^[A-Z]{3}$")],
        rate: float,
        db: AsyncSession = Depends(get_db)
) -> ExchangeRateSchema:
    dao_exchange = ExchangeDAO(db)
